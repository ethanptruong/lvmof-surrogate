...
def objective_xgb(trial, X, y, cv, groups):
    param = {
        ...
        "gamma":            trial.suggest_float("gamma", 0.05, 3.0),
...
def objective_xgb_cl_mi(trial, X, y, cv, groups):
    ...
    if is_best:
        print(f"\n  ↳ [CL+MI XGB] Trial {trial.number} per-fold:")
        print(f"    {'Fold':>6} {'QWK':>8} {'MAE':>8} {'ExactAcc':>10} {'Partial_n':>10}")
        all_splits = list(cv.split(X, y, groups))
        for i, (qwk, mae, acc) in enumerate(zip(fold_qwk, fold_mae, fold_acc)):
            val_idx   = all_splits[i][1]
...
def eval_pipe(name: str, pipe, X, y, cv, groups, scoring, n_jobs: int = 1):
    ...
    n_splits  = getattr(cv, "n_splits", 1)
    n_repeats = getattr(cv, "n_repeats", 1)

    def _stats(scores):
        if n_repeats > 1:
            repeat_means = scores.reshape(n_repeats, n_splits).mean(axis=1)
            return repeat_means.mean(), scores.std(ddof=1)
        return scores.mean(), scores.std(ddof=1)

    qwk_m, qwk_s = _stats(out["test_qwk"])
    mae_m, mae_s = _stats(out["test_mae"])
    w1_m,  w1_s  = _stats(out["test_within1"])
    acc_m, acc_s = _stats(out["test_exact_acc"])

    print(f"\n{name}")
    print(f"  QWK      {qwk_m:.4f}  ±{qwk_s:.4f}")
    print(f"  MAE      {-mae_m:.4f}  ±{mae_s:.4f}")
    print(f"  Within-1 {w1_m:.4f}  ±{w1_s:.4f}")
